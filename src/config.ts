...
export function parseOptions(input: Record<string, unknown> | undefined): Options {
  ...
  if (typeof host !== "string" || host.length === 0) throw new Error("sshopencode: host is required")
  if (/[\0\r\n]/.test(host)) throw new Error("sshopencode: host contains a forbidden control character")
  if (host.startsWith("-")) throw new Error("sshopencode: host cannot start with a dash")
  if (typeof root !== "string" || !path.posix.isAbsolute(root)) {
    throw new Error("sshopencode: root must be an absolute remote path")
  }
  if (/[\0\r\n]/.test(root)) throw new Error("sshopencode: root contains a forbidden control character")
  if (typeof sshBinary !== "string" || sshBinary.length === 0) {
    throw new Error("sshopencode: sshBinary must be a non-empty string")
  }
  if (!Array.isArray(sshArgs) || !sshArgs.every((item) => typeof item === "string" && !/[\0\r\n]/.test(item))) {
    throw new Error("sshopencode: sshArgs must be an array of strings")
  }
  if (!Number.isSafeInteger(maxOutputBytes) || Number(maxOutputBytes) < 1024) {
    throw new Error("sshopencode: maxOutputBytes must be an integer of at least 1024")
  }
