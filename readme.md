...
the github tool uses the local authenticated `gh` executable without a shell. no subcommand is filtered, including `gh auth token`.

`git` covers the local repository while `gh` covers the github api. both parse a command string into an argument array and execute the binary directly, so quoting is predictable and no shell is involved. that is the whole of it: no argument is filtered and no output is rewritten.

this is deliberate. `-c core.sshCommand=`, `-c diff.external=`, `--exec-path=` and a dozen relatives all execute arbitrary code, and a blocklist that catches four of them is theatre — it reads like a boundary while leaving the rest open. the full command is rendered into the approval prompt exactly as it will run, so the decision belongs there. `GIT_TERMINAL_PROMPT=0` remains, not as a restriction but because there is no terminal to prompt on and git would otherwise block until the tool timed out. a working directory outside the current worktree is included in the approval pattern so a remembered `git log` cannot be replayed against an unrelated repository.
